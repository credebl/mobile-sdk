...
export function ensureSingleCredentialRequest(result: CredentialsForProofRequest) {
  if (result.formattedSubmission.entries.length !== 1) {
    throw new Error(
      'Digital Credentials API supports only a single credential request'
    )
  }
}

export function applySelectedCredentialFilter(
  result: CredentialsForProofRequest,
  credentialId: string
) {
  ...
  const match = entry.credentials.find(
    (c) => c.credential.record.id === credentialId
  )

  if (!match) {
    throw new Error(
      `Selected credential '${credentialId}' was not found in the resolved credentials`
    )
  }
