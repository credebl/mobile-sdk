...
export function getCredentialBindingResolver({
  ...
  return async ({
    ...
    const shouldKeyBeHardwareBackedForSdJwtVc =
      (credentialConfiguration?.format === "vc+sd-jwt" ||
        credentialConfiguration.format === "dc+sd-jwt" ||
    ...
    const shouldKeyBeHardwareBacked =
      shouldKeyBeHardwareBackedForSdJwtVc ||
      shouldKeyBeHardwareBackedForMsoMdoc;
    ...
    if (!proofTypes.jwt || proofTypes.jwt.keyAttestationsRequired) {
      throw new Error(
        "Unable to request credentials. Only jwt proof type without key attestations supported",
      );
    }
    ...
    if (!algs?.length) {
      throw new Error(
        "No supported JWT signature algorithms provided by issuer",
      );
    }
    ...
    if (didMethod) {
      ...
      const didResults = await Promise.all(
        keys.map(async (key) => {
          const didResult = await didsApi.create<
            JwkDidCreateOptions | KeyDidCreateOptions
          >({
            method: dm,
            options: {
              keyId: key.keyId,
