...
function findDisplay<Display extends { locale?: string; lang?: string }>(
  ...
  let item = display.find(
    (d) => d.locale === preferredLocale || d.lang === preferredLocale,
  );

  if (!item && preferredLocale.includes("-")) {
    const languageOnly = preferredLocale.split("-")[0];
    item = display.find(
      (d) => d.locale === languageOnly || d.lang === languageOnly,
    );
  }
...
export function getCredentialDisplayWithDefaults(
...
export function getIssuerDisplay(
  ...
  const openidIssuerDisplay = findDisplay(
    Array.isArray(metadata?.issuer?.display)
      ? metadata.issuer.display
      : undefined,
    preferredLocale,
  );
  ...
  const openidCredentialDisplay = findDisplay(
    Array.isArray(metadata?.credential?.display)
      ? metadata.credential.display
      : undefined,
    preferredLocale,
  );

  if (
    openidCredentialDisplay &&
    !issuerDisplay.logo?.url &&
    openidCredentialDisplay.logo
  ) {
    issuerDisplay.logo = {
...
function getW3cIssuerDisplay(
  ...
  if (openId4VcMetadata) {
    ...
    const openidCredentialDisplay = findDisplay(
      openId4VcMetadata.credential.display,
    );
    if (
      openidCredentialDisplay &&
      !issuerDisplay.logo &&
  ...
  const issuerJson =
    typeof jffCredential.issuer === "string" ? undefined : jffCredential.issuer;
  ...
  if (!issuerDisplay.logo || !issuerDisplay.logo.url) {
    if (issuerJson?.logoUrl) {
      ...
      issuerDisplay.logo = {
        url:
          typeof issuerJson.image === "string"
            ? issuerJson.image
            : issuerJson.image.id,
      };
    }
...
export function getSdJwtIssuerDisplay(
  openId4VcMetadata?: OpenId4VcCredentialMetadata | null,
): CredentialIssuerDisplay {
  const issuerDisplay: Partial<CredentialIssuerDisplay> =
    getIssuerDisplay(openId4VcMetadata);
...
export function getW3cCredentialDisplay(
  ...
  const credentialDisplay: Partial<CredentialDisplay> = getCredentialDisplay(
    credential,
    openId4VcMetadata,
  );
  ...
  if (!credentialDisplay.name && jffCredential.type.length > 1) {
    ...
    credentialDisplay.name =
      lastType && !lastType.startsWith("http")
        ? sanitizeString(lastType)
        : undefined;
  }
...
export function getSdJwtTypeMetadataCredentialDisplay(
  ...
  const typeMetadataDisplay = findDisplay(
    sdJwtTypeMetadata.display,
    preferredLocale,
  );
...
export function getSdJwtCredentialDisplay(
  ...
  if (typeMetadata) {
    credentialDisplay = getSdJwtTypeMetadataCredentialDisplay(
      typeMetadata,
      preferredLocal,
    );
  } else if (openId4VcMetadata) {
    credentialDisplay = getOpenId4VcCredentialDisplay(
      openId4VcMetadata,
      preferredLocal,
    );
  }
...
export function getDisclosedAttributeNamesForDisplay(
  ...
  if (credential.credential.claimFormat === ClaimFormat.MsoMdoc) {
    return Array.from(
      new Set(
        credential.disclosed.paths.map((path) => sanitizeString(path[1])),
      ),
    );
  }
...
export function getCredentialForDisplay(
  ...
  if (credentialRecord instanceof W3cCredentialRecord) {
    ...
    const credentialDisplay = getW3cCredentialDisplay(
      credential,
      openId4VcMetadata,
    );
...
export function getOpenId4VcIssuerDisplay(
  ...
  if (openId4VcMetadata) {
    const openidIssuerDisplay = findDisplay(
      openId4VcMetadata.issuer.display,
      preferredLocale,
    );
...
export function getAttributesAndMetadataForMdocPayload(
  ...
  const attributes: CredentialForDisplay["attributes"] = Object.fromEntries(
    Object.values(namespaces).flatMap((v) => {
      return Object.entries(v).map(([key, value]) => [
        key,
        recursivelyMapAttributes(value),
      ]);
    }),
  );
